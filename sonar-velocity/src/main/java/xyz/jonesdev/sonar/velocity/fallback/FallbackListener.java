  ...
  public void handle(final @NotNull PreLoginEvent event) throws Throwable {
    ...
    if (fallback.getBlacklisted().has(inetAddress.toString())) {
      ...
      initialConnection.getConnection().closeWith(Disconnect.create(
        Sonar.get().getTranslations().getVerification().getBlacklisted(),
    ...
    if (fallback.getConnected().containsKey(event.getUsername())
      ...
      initialConnection.getConnection().closeWith(Disconnect.create(
        Sonar.get().getTranslations().getVerification().getAlreadyVerifying(),
        inboundConnection.getProtocolVersion()
      ));
      return;
    }
    ...
    if (fallback.getConnected().size() >= Sonar.get().getConfig().getVerification().getMaxVerifyingPlayers()) {
      initialConnection.getConnection().closeWith(Disconnect.create(
        Sonar.get().getTranslations().getVerification().getTooManyPlayers(),
        inboundConnection.getProtocolVersion()
      ));
      return;
    }
    ...
    if (!fallback.getRatelimiter().attempt(inetAddress)) {
      initialConnection.getConnection().closeWith(Disconnect.create(
        Sonar.get().getTranslations().getVerification().getTooFastReconnect(),
        inboundConnection.getProtocolVersion()
      ));
      return;
    }
    ...
    if (fallback.getQueue().getQueuedPlayers().containsKey(inetAddress)) {
      initialConnection.getConnection().closeWith(Disconnect.create(
        Sonar.get().getTranslations().getVerification().getAlreadyQueued(),
        inboundConnection.getProtocolVersion()
      ));
      return;
    }
    ...
    channel.eventLoop().execute(() -> {
      ...
      fallback.getQueue().queue(inetAddress, () -> channel.eventLoop().execute(() -> {
        ...
        if (!Sonar.get().getConfig().getVerification().getValidNameRegex().matcher(event.getUsername()).matches()) {
          mcConnection.closeWith(Disconnect.create(Sonar.get().getTranslations().getVerification().getInvalidUsername(), mcConnection.getProtocolVersion()));
          return;
        }
  ...
  public void handle(final @NotNull LoginEvent event) {
    ...
    if (Sonar.get().getConfig().getLockdown().isEnabled()) {
      if (!event.getPlayer().hasPermission("sonar.lockdown")) {
        connectedPlayer.getConnection().closeWith(Disconnect.create(
          Sonar.get().getTranslations().getLockdown().getDisconnect(), connectedPlayer.getProtocolVersion()
        ));

        if (Sonar.get().getConfig().getLockdown().isLogAttempts()) {
          Sonar.get().getLogger().info(
            Sonar.get().getTranslations().getLockdown().getConsoleLog()
              .replace("%player%", event.getPlayer().getUsername())
              .replace("%ip%", Sonar.get().getConfig()
                .getAddressParser().apply(event.getPlayer().getRemoteAddress().getAddress()))
              .replace("%protocol%",
        ...
        event.getPlayer().sendMessage(Component.text(Sonar.get().getTranslations().getLockdown().getNotification()));
      }
    }
    ...
    if (maxOnlinePerIp > 0) {
      ...
      if (onlinePerIp >= maxOnlinePerIp) {
        connectedPlayer.getConnection().closeWith(Disconnect.create(
          Sonar.get().getTranslations().getTooManyOnlinePerIP(), connectedPlayer.getProtocolVersion()
        ));
      }
    }
  }
