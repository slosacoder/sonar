...
public final class FallbackVerificationHandler implements FallbackPacketListener {
  ...
  private static boolean validateClientBrand(final @NotNull FallbackUser<?, ?> user, final ByteBuf content) {
    ...
    try {
      ...
      final String read = ProtocolUtil.readString(content, Sonar.get().getConfig().getVerification().getMaxBrandLength());
  ...
  public void handle(final @NotNull FallbackPacket packet) {
    ...
    final int maxPackets = Sonar.get().getConfig().getVerification().getMaxLoginPackets()
      + Sonar.get().getConfig().getVerification().getMaxMovementTick();
    ...
    final long timeout = Sonar.get().getConfig().getVerification().getTimeout();
    ...
    if (packet instanceof Transaction) {
      ...
      if (!Sonar.get().getConfig().getVerification().isCheckClientGravity()) {
        finish();
        return;
      }
  ...
  private void handlePositionUpdate(final double x, final double y, final double z, final boolean ground) {
    ...
    if (deltaY > 0.07) {
      ...
      if (movementTick++ >= MAX_MOVEMENT_TICK) {
        if (Sonar.get().getConfig().getVerification().isCheckClientCollisions()) {
  ...
  private void finish() {
    ...
    user.disconnect(Sonar.get().getTranslations().getVerification().getSuccess());

    user.getFallback().getLogger().info(Sonar.get().getTranslations().getVerification().getSuccessLogMessage()
