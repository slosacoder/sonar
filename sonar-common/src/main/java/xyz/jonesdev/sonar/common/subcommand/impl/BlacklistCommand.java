    ...
    switch (invocation.getRawArguments()[1].toLowerCase()) {
      case "add": {
        if (invocation.getRawArguments().length <= 2) {
          invocation.getSender().sendMessage(SONAR.getTranslations().getCommands().getIncorrectUsage()
        ...
        if (!IP_PATTERN.matcher(rawInetAddress).matches()) {
          invocation.getSender().sendMessage(SONAR.getTranslations().getCommands().getInvalidIPAddress());
          return;
        }
        ...
        try {
          ...
          if (inetAddress.isAnyLocalAddress() || inetAddress.isLoopbackAddress()) {
            invocation.getSender().sendMessage(SONAR.getTranslations().getCommands().getIllegalIPAddress());
            return;
          }
        ...
        if (SONAR.getFallback().getBlacklisted().has(inetAddress.toString())) {
          invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_DUPLICATE);
          return;
        }

        if (SONAR.getVerifiedPlayerController().has(inetAddress)) {
          invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_ADD_WARNING
            .replace("%ip%", rawInetAddress));
        }

        SONAR.getFallback().getBlacklisted().put(inetAddress.toString());
        invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_ADD
          .replace("%ip%", rawInetAddress));
        break;
      }
      ...
      case "remove": {
        ...
        if (!SONAR.getFallback().getBlacklisted().has(inetAddress.toString())) {
          invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_NOT_FOUND);
          return;
        }

        SONAR.getFallback().getBlacklisted().invalidate(inetAddress.toString());
        invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_REMOVE
          .replace("%ip%", rawInetAddress));
        break;
      }
      ...
      case "clear": {
        ...
        if (blacklisted == 0) {
          invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_EMPTY);
          return;
        }
        ...
        invocation.getSender().sendMessage(
          SONAR.getTranslations().BLACKLIST_CLEARED
      ...
      case "size": {
        invocation.getSender().sendMessage(SONAR.getTranslations().BLACKLIST_SIZE
