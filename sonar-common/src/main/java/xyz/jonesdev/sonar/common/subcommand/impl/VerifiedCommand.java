    ...
    switch (invocation.getRawArguments()[1].toLowerCase()) {
      case "history": {
        ...
        if (!SONAR.getVerifiedPlayerController().has(realInetAddress)) {
          invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_NOT_FOUND);
          return;
        }
      ...
      case "remove": {
        ...
        if (LOCK.contains(realInetAddress)) {
          invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_BLOCKED);
          return;
        }
        ...
        invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_REMOVE
      ...
      case "clear": {
        ...
        if (verified == 0) {
          invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_EMPTY);
          return;
        }

        SONAR.getVerifiedPlayerController().clearAll();

        invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_CLEARED
          .replace("%removed%", Sonar.DECIMAL_FORMAT.format(verified)));
        break;
      }

      case "size": {
        invocation.getSender().sendMessage(SONAR.getTranslations().VERIFIED_SIZE
