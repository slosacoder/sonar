...
public final class FallbackSessionHandler implements MinecraftSessionHandler {
  ...
  FallbackSessionHandler(final Fallback fallback,
    ...
    if (Sonar.get().getConfig().getVerification().isLogConnections()) {
      ...
      if (!fallback.isPotentiallyUnderAttack() || Sonar.get().getConfig().getVerification().isLogConnectionsDuringAttack()) {
        fallback.getLogger().info(Sonar.get().getTranslations().getVerification().getConnectionLogMessage()
          .replace("%name%", connectedPlayer.getUsername())
          .replace("%ip%", Sonar.get().getConfig().getAddressParser().apply(fallbackPlayer.getInetAddress()))
